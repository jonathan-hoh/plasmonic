...
sec_convert = (7812.5*2)**(-1) #number of 2^23 accum dumps needed for one sec of integration
ms_convert = 0.064 #same as above, just for a ms of integration

t_1ms = round(1/ms_convert)
t_30ms = round(30/ms_convert)
t_300ms = round(300/ms_convert)
t_1000ms = round(1000/ms_convert)
...
def accum_test(bin):
        ...
        while (count < t_1000ms)
            ...
            I = accum_data[0::2]
            I = I[2:][bin]
            mag = 10*np.log10(mag+1e-20)
            vals[count] = I
            count += 1
        vals = 10*np.log10(vals+1e-20)     
        accums = np.zeros(4)
        accums[0] = np.sum(vals[:t_1ms])/t_1ms
        accums[1] = np.sum(vals[:t_30ms])/t_30ms
        accums[2] = np.sum(vals[:t_300ms])/t_300ms
        accums[3] = np.sum(vals)/t_1000ms
        
        print('1ms at %d, 30ms at %d, 300ms at %d, 1sec at %d' % (accums[0], accums[1], accums[2], accums[3])
